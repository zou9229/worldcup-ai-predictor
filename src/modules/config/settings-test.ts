...
export async function runTest(
  ...
  try {
    switch (group) {
      ...
      case 'gemini_vertex':
        return await testGeminiVertex(inputs, configs);
      case 'replicate':
...
async function testGeminiVertex(inputs: Record<string, string>, configs: Record<string, string>): Promise<TestResult> {
  const missing = need(configs, ['gemini_vertex_service_account_json']);
  if (missing) return { success: false, message: missing };

  const model = inputs.model || configs.gemini_vertex_model || 'gemini-2.5-flash';
  const location = configs.gemini_vertex_location || 'us-central1';
  ...
  return {
    success: true,
    message: 'Vertex AI Gemini accepted the request',
    details: {
      Model: result.model,
      Location: location,
      Reply: result.text.slice(0, 200) || '(empty)',
    },
  };
}
